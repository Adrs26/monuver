...
class UpdateBudgetingUseCase(
    private val budgetingRepository: BudgetingRepository,
    private val transactionRepository: TransactionRepository
) {
    suspend operator fun invoke(budgetingState: EditBudgetingContentState): DatabaseResultMessage {
        if (budgetingState.maxAmount == 0L) {
            return DatabaseResultMessage.EmptyBudgetingMaxAmount
        }
        ...
        budgetingRepository.updateBudgeting(budgeting)
        return DatabaseResultMessage.UpdateBudgetingSuccess
    }
}