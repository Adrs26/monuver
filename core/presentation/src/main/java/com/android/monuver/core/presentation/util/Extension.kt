...
fun TextFieldValue.toRupiahFieldValue(): Pair<Long, TextFieldValue> {
    val cleanInput = text.replace(Regex("\\D"), "")
    val balance = cleanInput.toLongOrNull() ?: 0L

    val formattedText = balance.toRupiah()
...
fun DatabaseResultState.showToast(context: Context) {
    when(this) {
        ...
        DatabaseResultState.CreateAccountSuccess -> {
            context.getString(R.string.account_successfully_created).showMessageWithToast(context)
        }
