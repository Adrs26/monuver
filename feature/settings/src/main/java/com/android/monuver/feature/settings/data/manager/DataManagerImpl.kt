package com.android.monuver.feature.settings.data.manager
...
internal class DataManagerImpl(
    val context: Context,
    val customDispatcher: CustomDispatcher
) : DataManager {
    override suspend fun backupData(data: DataBackup) {
        val fileName = "monuver_backup_${System.currentTimeMillis()}.json"
        val backupDataJson = Gson().toJson(data)

        withContext(customDispatcher.io) {
    ...
    override suspend fun restoreData(stringUri: String): DataBackup {
        val backupDataJson = withContext(customDispatcher.io) {
            context.contentResolver.openInputStream(stringUri.toUri())
                ?.bufferedReader().use { it?.readText() }
        }
        return Gson().fromJson(backupDataJson, DataBackup::class.java)
    }

    override suspend fun exportDataToPdf(
        reportName: String,
        username: String,
        startDate: String,
        endDate: String,
        ...
        withContext(customDispatcher.io) {
            try {
                val fileName = "monuver_report_${System.currentTimeMillis()}.json"
    ...
    private fun createPdfDocument(
        ...
        val startPeriod = DateHelper.formatToReadable(startDate)
        val endPeriod = DateHelper.formatToReadable(endDate)
